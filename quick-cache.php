...
*/
else if (is_admin () || ($_GET["preview"] && $_GET["template"]))
	{
		if (!version_compare (PHP_VERSION, "5.2", ">="))
			...
			{
				register_shutdown_function (create_function ('', 'echo \'<script type="text/javascript">alert(\\\'You need WordPress® 2.9.2 or higher to use the Quick Cache plugin.\\\');</script>\';'));
			}
		else if (basename (dirname (__FILE__)) === basename (WPMU_PLUGIN_DIR))
			{
				register_shutdown_function (create_function ('', 'echo \'<script type="text/javascript">alert(\\\'The Quick Cache plugin is compatible with WordPress® Multisite. However, the Quick Cache plugin should NOT be in the /' . basename (WPMU_PLUGIN_DIR) . ' directory. Please move it into the standard /plugins directory, then re-activate.\\\');</script>\';'));
			}
	}
?>
...
Tags: cache, quick cache, quick-cache, quickcache, speed, performance, loading, generation, execution, benchmark, benchmarking, debug, debugging, caching, cash, caching, cacheing, super cache, advanced cache, advanced-cache, wp-cache, wp cache, multisite, options panel included, websharks framework, w3c validated code, includes extensive documentation, highly extensible
...
WordPress® Multisite/Networking is a special version of WordPress®. If Quick Cache is installed under a Multisite/Networking installation, it will be enabled for ALL blogs the same way. The centralized config options for Quick Cache, can only be modified by a Super Administrator operating on the main site with Blog ID# 1. Quick Cache has internal processing routines that prevent configuration changes, including menu displays; for anyone other than a Super Administrator operating on the main site with Blog ID# 1.
...
= 2.1.7 =
Upgrade recommended. Bug fixed in remote calls affecting MD5 Salt validation on some servers.

= 2.1.6 =
Upgrade recommended. Quick Cache is now compatible with WordPress® 3.0, even with Multisite/Networking enabled.

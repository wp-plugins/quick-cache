...
Tags: options panel included, websharks framework, w3c validated code, includes extensive documentation, highly extensible
...
1. Navigate to the `Quick Cache` panel & enable it.
...
This effectively creates two versions of the cache. When iPhone browsers are detected, Quick Cache will prepend `IPHONES` to the `REQUEST_URI`, before it generates the MD5 hash for storage.
...
This effectively creates two versions of the cache. When `BlueLizardsCookie` contains `BlueLizards`, Quick Cache will prepend `BlueLizards` to the `REQUEST_URI`, before it generates the MD5 hash for storage. Another, even simpler way to handle this, would be to use the value of a specific cookie to generate multiple variations of the cache. So instead of the Ternary expression shown above, you would simply set your Version Salt to:

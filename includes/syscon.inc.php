...
*/
if (($GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["is_multisite"] = ((function_exists ("is_multisite") && is_multisite ()) || function_exists ("wpmu_current_site"))) && ((defined ("MULTISITE_FARM") && MULTISITE_FARM) || file_exists (dirname (dirname (__FILE__)) . "/multisite.farm")))
	$GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["is_multisite_farm"] = true;
/*
Determine the full url to the directory this plugin resides in.
*/
$GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"] = content_url () . preg_replace ("/^(.*?)(\/" . preg_quote (basename (WP_CONTENT_DIR), "/") . ")/", "", preg_replace ("/" . preg_quote (DIRECTORY_SEPARATOR, "/") . "/", "/", dirname (dirname (__FILE__))));
/*
...
*/
$GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["menu_pages"] = array ("installation" => false, "tools" => true, "support" => true, "donations" => true);
/*
...
function ws_plugin__qcache_configure_options_and_their_defaults ($options = FALSE) /* Can also be used to merge options with defaults. */
	{
		...
		/**/
		"dont_cache_these_uris" => "wp-app\nwp-admin\nwp-login\nxmlrpc.php", /* URI patterns NOT to cache. */
		...
		foreach ($GLOBALS["WS_PLUGIN__"]["qcache"]["o"] as $key => &$value)
			{
				...
				/**/
					...
					$value = $default_options[$key];
				/**/
				else if ($key === "dont_cache_these_uris" && !is_string ($value)) /* Allowed to be non-default empty. */
					$value = $default_options[$key];
				/**/

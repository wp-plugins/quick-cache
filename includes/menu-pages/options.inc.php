...
echo '<p>The Auto-Cache Engine, is designed to combat this issue, by taking on the responsibility of being that first visitor to a page that has not yet been cached, or... has an expired cache. The Auto-Cache Engine is powered, in part, by <a href="http://codex.wordpress.org/Category:WP-Cron_Functions" target="_blank" rel="xlink">WP Cron</a> ( already built into the WP core framework ), and it also uses the <a href="http://php.net/manual/en/book.curl.php" target="_blank" rel="xlink">cURL</a> extension for PHP, which is usually installed with PHP by default. If you do NOT see a red error message below, your server meets these basic requirements.</p>' . "\n";
echo '<p>The Auto-Cache Engine, obtains its list of URLs, to "Auto Cache", from two different sources. It can read your <a href="http://wordpress.org/extend/plugins/google-sitemap-generator/" target="_blank" rel="xlink">XML Sitemap</a>, and/or a list of specific URLs that you supply. If you supply both sources, it will use both sources collectively, and intuitively. The Auto-Cache engine takes ALL of your other configuration options into consideration, including your Expiration setting, as well as any No-Cache rules.</p>' . "\n";
...
echo (!function_exists ("curl_init")) ? '<p class="ws-menu-page-error"><a href="http://php.net/manual/en/book.curl.php" target="_blank" rel="xlink">cURL</a> is NOT installed. Your server must support cURL in order to use the Auto-Cache Engine.</p>' . "\n" : '';
/**/
echo '<table class="form-table">' . "\n";
echo '<tbody>' . "\n";
echo '<tr>' . "\n";
...
echo '<strong>Tip:</strong> the <a href="http://wordpress.org/extend/plugins/google-sitemap-generator/" target="_blank" rel="xlink">Google® XML Sitemaps</a> plugin for WordPress®, will keep this up-to-date automatically :-)' . "\n";
echo '</td>' . "\n";
/**/
echo '</tr>' . "\n";
echo '<tr>' . "\n";
...
echo 'A log will be maintained at: <code>/wp-content/cache/auto-cache.log</code><br />' . "\n";
...
echo 'Run De-Activation Routines for Quick Cache?' . "\n";
echo '</label>' . "\n";
echo '</th>' . "\n";
/**/
echo '</tr>' . "\n";
...
echo '<option value="1"' . (($GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["run_deactivation_routines"]) ? ' selected="selected"' : '') . '>Yes ( erase all Quick Cache data/options on de-activation )</option>' . "\n";
echo '<option value="0"' . ((!$GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["run_deactivation_routines"]) ? ' selected="selected"' : '') . '>No ( safeguard all Quick Cache data/options )</option>' . "\n";
echo '</select><br />' . "\n";
echo 'It is recommended that you Safeguard all Quick Cache data/options on your system.' . "\n";
echo '</td>' . "\n";
/**/
echo '</tr>' . "\n";
echo '</tbody>' . "\n";
...
/**/
echo '<form method="post" name="ws_plugin__qcache_clear_form" id="ws-plugin--qcache-clear-form" style="margin:0 0 25px 25px;">' . "\n";
...
echo '<input type="submit" value="Clear Cache Manually" class="ws-menu-page-center" />' . "\n";
echo '</form>' . "\n";
/**/

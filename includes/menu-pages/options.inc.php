...
/**/
echo '<p><strong>Cloud Computing?</strong> If your site is hosted on a Cloud Computing model ( such as the Rackspace® Cloud or Media Temple ), you should set this to <em>flock</em> unless they tell you otherwise.</p>' . "\n";
/**/
echo '<table class="form-table">' . "\n";
echo '<tbody>' . "\n";
echo '<tr>' . "\n";
...
echo '<p>This is for advanced users only. Alright, here goes... Quick Cache stores its cache files using an md5() hash of the URI that it is caching. If you want to build these hash strings out of something other than just the URI by itself, you can add a Salt to the mix. So instead of just <code>md5($_SERVER["REQUEST_URI"])</code>, you might have <code>md5($_COOKIE["myCookie"].$_SERVER["REQUEST_URI"])</code>. This would create multiple versions of each page depending on the value of <code>$_COOKIE["myCookie"]</code>. If <code>$_COOKIE["myCookie"]</code> is empty, then just the <code>$_SERVER["REQUEST_URI"]</code> is used. So you see, this gives you the ability to dynamically create multiple variations of the cache, and those dynamic versions of the cache will be served on subsequent visits.</p>' . "\n";
echo '<p>A Salt can be a single variable like <code>$_SERVER["HTTP_HOST"]</code>, or it can be a combination of multiple variables, like <code>$_SERVER["HTTPS"].$_SERVER["HTTP_HOST"].$_COOKIE["myCookie"]</code>. When using multiple variables, please separate them with a dot, like the example. Experts can use PHP ternary expressions that evaluate into something. For example: <code>((preg_match("/IPHONE/i", $_SERVER["HTTP_USER_AGENT"])) ? "IPHONES" : "")</code>. This would force a separate version of the cache to be created for iPhone browsers. With this method your possibilities are limitless.</p>' . "\n";
...
/**/
echo '<td>' . "\n";
echo 'md5(<input type="text" name="ws_plugin__qcache_version_salt" id="ws-plugin--qcache-version-salt" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["version_salt"]) . '" style="width:300px;" />.$_SERVER["REQUEST_URI"])<br />' . "\n";
echo 'You can use Super Globals: $_SERVER, $_GET, $_REQUEST, $_COOKIE, etc. Or Constants defined in wp-config.php. Example: <code>DB_NAME.DB_HOST.$_SERVER["HTTP_HOST"]</code> ( separate multiple variables with a dot ). Your Salt will be checked for PHP syntax errors. If syntax errors are found, you\'ll receive a JavaScript alert, after you click Save.<br />' . "\n";
echo '</td>' . "\n";
/**/
echo '</tr>' . "\n";
echo '</tbody>' . "\n";

		...
		function ws_plugin__qcache_clear_on_post_page_creations_deletions ($id = FALSE)
			{
				...
				/**/
				if (ws_plugin__qcache_is_blog_admin () && in_array ($pagenow, ($pages = array ("edit.php", "post.php", "post-new.php"))))
					{
										...
										if ($cache) /* If a cache file exists for this $host_uri. */
											{
												if (is_writable ($cache) && unlink ($cache))
													{
														if (!is_multisite () || !ws_plugin__qcache_is_multisite_farm () || is_main_site ())
															{
		...
		function ws_plugin__qcache_clear_on_creations_deletions ($term_or_link_id = FALSE)
			{
				...
				unset ($__refs, $__v); /* Unset defined __refs, __v. */
				/**/
				if (ws_plugin__qcache_is_blog_admin () && in_array ($pagenow, ($pages = array ("edit-tags.php", "link-manager.php", "link.php", "link-add.php", "edit-link-categories.php", "link-category.php"))))
					{
		...
		function ws_plugin__qcache_clear_on_theme_change ()
			{
				...
				/**/
				if (ws_plugin__qcache_is_blog_admin () && in_array ($pagenow, ($pages = array ("themes.php"))))
					{
				...
				if (($nonce = $_POST["ws_plugin__qcache_ajax_clear"]) && wp_verify_nonce ($nonce, "ws-plugin--qcache-ajax-clear"))
					{
						if (is_multisite () && !is_main_site () && is_object ($current_blog) && $current_blog->blog_id)
							{
								ws_plugin__qcache_purge_cache_dir($current_blog);
								/**/
							...
							{
								ws_plugin__qcache_purge_cache_dir ();
								/**/
								header ("Content-Type: text/plain; charset=utf-8");
								/**/

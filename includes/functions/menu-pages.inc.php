		...
		function ws_plugin__qcache_update_all_options ()
			{
				...
				if (($nonce = $_POST["ws_plugin__qcache_options_save"]) && wp_verify_nonce ($nonce, "ws-plugin--qcache-options-save"))
								...
								if ($GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["enabled"]) /* Enabled? */
									{
										...
										if (!$enabled) /* Otherwise, we need to throw a warning up. The site owner needs to try again. */
											{
												if (!function_exists ("wp_cron")) /* Special warning for lack of the wp_cron function. Disabled by some web hosts. */
													{
														$notice = '<strong>Error:</strong> Could NOT enable Quick Cache. Your installation of WordPress® is missing the <code>wp_cron</code> function. Some web hosts disable this intentitally. Please contact your web host for assistance.';
														ws_plugin__qcache_admin_notices (); /* Flush all notices. */
														ws_plugin__qcache_display_admin_notice ($notice, true);
													}
												else /* Otherwise, the error must have something to do with file/dir permissions. */
													{
														$notice = '<strong>Error:</strong> Could NOT enable Quick Cache. Please check permissions on <code>/wp-config.php</code>, <code>/wp-content/</code> and <code>/wp-content/cache/</code>. Permissions need to be <code>755</code> or higher.';
														ws_plugin__qcache_admin_notices (); /* Flush all notices. */
														ws_plugin__qcache_display_admin_notice ($notice, true);
													}
											}
										...
										if (!$disabled) /* Otherwise, we need to throw a warning up. The site owner needs to try again. */
											{
												if (!function_exists ("wp_cron")) /* Special warning for lack of the wp_cron function. Disabled by some web hosts. */
													{
														$notice = '<strong>Error:</strong> Could NOT disable Quick Cache. Your installation of WordPress® is missing the <code>wp_cron</code> function. Some web hosts disable this intentitally. Please contact your web host for assistance.';
														ws_plugin__qcache_admin_notices (); /* Flush all notices. */
														ws_plugin__qcache_display_admin_notice ($notice, true);
													}
												else /* Otherwise, the error must have something to do with file/dir permissions. */
													{
														$notice = '<strong>Error:</strong> Could NOT disable Quick Cache. Please check permissions on <code>/wp-config.php</code>, <code>/wp-content/</code> and <code>/wp-content/cache/</code>. Permissions need to be <code>755</code> or higher.';
														ws_plugin__qcache_admin_notices (); /* Flush all notices. */
														ws_plugin__qcache_display_admin_notice ($notice, true);
													}
											}
					...
					{
						if (ws_plugin__qcache_purge_cache_dir ()) /* Purge the cache in real-time. */
							{
								$notice = '<strong>Cache cleared ( purged in real-time ).</strong>';
								ws_plugin__qcache_display_admin_notice ($notice);
							}
						else /* Otherwise, there must have been a problem during the purge. */
							{
								$notice = '<strong>Unable to purge the cache, please try again.</strong>';
								ws_plugin__qcache_display_admin_notice ($notice, true);
							}
					}
				/**/
				...
				if (!is_multisite () || (is_multisite () && is_super_admin ()))
					{
						...
						echo '<input type="button" id="ws-plugin--qcache-ajax-clear" style="outline:none; min-width:165px; padding-left:22px; padding-right:5px; background-repeat:no-repeat; background-position:5px center; background-image:url(\'' . ws_plugin__qcache_esc_sq ($GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"]) . '/images/ajax-clear.png\');" value="Clear Quick Cache" onclick="jQuery (this).css (\'background-image\', \'url(\\\'' . ws_plugin__qcache_esc_sq ($GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"]) . '/images/ajax-loader.gif\\\')\'); jQuery.post (ajaxurl, {action: \'ws_plugin__qcache_ajax_clear\', ws_plugin__qcache_ajax_clear: \'' . ws_plugin__qcache_esc_sq (wp_create_nonce ("ws-plugin--qcache-ajax-clear")) . '\'}, function (response){ eval (response); });" />' . "\n";
						echo '</form>';
						/**/

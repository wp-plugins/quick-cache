...
if (!function_exists ("ws_plugin__qcache_update_all_options"))
	{
		function ws_plugin__qcache_update_all_options ($new_options = FALSE, $verified = FALSE, $update_other = TRUE, $display_notices = TRUE, $enqueue_notices = FALSE)
			{
						...
						if (!is_multisite () || (is_main_site () && is_super_admin ())) /* If Multisite, this MUST be ( Main Site / Super Admin ). */
							{
								...
								$new_options = ws_plugin__qcache_trim_deep (stripslashes_deep ($new_options));
								/**/
										...
										if (ws_plugin__qcache_add_wp_cache ()) /* Add WP_CACHE to the config file. */
											if (ws_plugin__qcache_add_advanced ()) /* Add the advanced-cache.php file. */
												if (ws_plugin__qcache_add_garbage_collector ()) /* Add the garbage collector. */
													if (ws_plugin__qcache_schedule_cache_dir_purge ()) /* Purge the cache. */
														{
															$enabled = true; /* Mark this variable as enabled successfully. */
															($options["auto_cache_enabled"]) ? ws_plugin__qcache_add_auto_cache_engine () : ws_plugin__qcache_delete_auto_cache_engine ();
															if (($display_notices === true || in_array ("success", (array)$display_notices)) && ($notice = '<strong>Options updated</strong>, and the cache was reset to avoid conflicts.'))
																($enqueue_notices === true || in_array ("success", (array)$enqueue_notices)) ? ws_plugin__qcache_enqueue_admin_notice ($notice) : ws_plugin__qcache_display_admin_notice ($notice);
														}
										/**/
												...
												if (!function_exists ("wp_cron")) /* Special warning for lack of the wp_cron function. */
													{
														if (($display_notices === true || in_array ("success", (array)$display_notices)) && ($notice = '<strong>Error:</strong> Could NOT enable Quick Cache. Your installation of WordPress® is missing the <code>wp_cron</code> function. Some web hosts disable this intentionally. Please contact your web host for assistance.'))
															($enqueue_notices === true || in_array ("success", (array)$enqueue_notices)) ? ws_plugin__qcache_enqueue_admin_notice ($notice, false, true) : ws_plugin__qcache_display_admin_notice ($notice, true);
													}
													...
													if (ws_plugin__qcache_schedule_cache_dir_purge ()) /* Purge the cache. */
														{
															...
															if (($display_notices === true || in_array ("success", (array)$display_notices)) && ($notice = '<strong>Options updated.</strong> Quick Cache disabled.'))
																($enqueue_notices === true || in_array ("success", (array)$enqueue_notices)) ? ws_plugin__qcache_enqueue_admin_notice ($notice) : ws_plugin__qcache_display_admin_notice ($notice);
														}
										/**/
...
*/
if (!function_exists ("ws_plugin__qcache_add_admin_header_controls"))
	{
		function ws_plugin__qcache_add_admin_header_controls ()
			{
				...
				if (!is_multisite () || (is_multisite () && is_super_admin ()))
					{
						...
						echo '<input type="button" id="ws-plugin--qcache-ajax-clear" style="outline:none; min-width:115px; padding-left:22px; padding-right:5px; background-repeat:no-repeat; background-position:5px center; background-image:url(\'' . ws_plugin__qcache_esc_sq ($GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"]) . '/images/ajax-clear.png\');" value="Clear Cache" title="Clear Cache Manually" onclick="jQuery (this).css (\'background-image\', \'url(\\\'' . ws_plugin__qcache_esc_sq ($GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"]) . '/images/ajax-loader.gif\\\')\'); jQuery.post (ajaxurl, {action: \'ws_plugin__qcache_ajax_clear\', ws_plugin__qcache_ajax_clear: \'' . ws_plugin__qcache_esc_sq (wp_create_nonce ("ws-plugin--qcache-ajax-clear")) . '\'}, function (response){ eval (response); });" />' . "\n";
						echo '</form>';
						/**/
				...
				if ($_GET["page"] && preg_match ("/ws-plugin--qcache-/", $_GET["page"]))
					{
						...
						wp_enqueue_script ("json-functions", $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"] . "/includes/menu-pages/json-functions.js", array (), $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["options_version"] . $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["filemtime"]);
						wp_enqueue_script ("jquery-ui-effects", $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"] . "/includes/menu-pages/jquery-ui-effects.js", array ("jquery", "jquery-ui-core"), $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["options_version"] . $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["filemtime"]);
						wp_enqueue_script ("ws-plugin--qcache-menu-pages", get_bloginfo ("wpurl") . "/?ws_plugin__qcache_menu_pages_js=1", array ("jquery", "thickbox", "media-upload", "json-functions", "jquery-ui-core", "jquery-ui-effects"), $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["options_version"] . $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["filemtime"]);
						/**/
				...
				if ($_GET["page"] && preg_match ("/ws-plugin--qcache-/", $_GET["page"]))
					{
						...
						wp_enqueue_style ("ws-plugin--qcache-menu-pages", get_bloginfo ("wpurl") . "/?ws_plugin__qcache_menu_pages_css=1", array ("thickbox"), $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["options_version"] . $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["filemtime"], "all");
						/**/
				...
				if ($_GET["ws_plugin__qcache_menu_pages_css"] && is_user_logged_in () && current_user_can ("edit_plugins"))
					{
						...
						$i = $GLOBALS["WS_PLUGIN__"]["qcache"]["c"]["dir_url"] . "/images";
						/**/
						ob_start ("ws_plugin__qcache_compress_css"); /* Compress. */
						/**/
		...
		function ws_plugin__qcache_options_page ()
			{
				...
				/**/
				ws_plugin__qcache_update_all_options () . clearstatcache ();
				/**/
				if (file_exists (ABSPATH . "wp-config.php") && !is_writable (ABSPATH . "wp-config.php"))
					{
						ws_plugin__qcache_display_admin_notice ('<strong>Permissions:</strong> Please check permissions on <code>' . esc_html (preg_replace ("/^" . preg_quote ($_SERVER["DOCUMENT_ROOT"], "/") . "/", "", ABSPATH . "wp-config.php")) . '</code>. Quick Cache needs write-access to this file. Permissions need to be <code>755</code> or higher.', true);
					}
				else if (file_exists (dirname (ABSPATH) . "/wp-config.php") && !is_writable (dirname (ABSPATH) . "/wp-config.php"))
					{
						ws_plugin__qcache_display_admin_notice ('<strong>Permissions:</strong> Please check permissions on <code>' . esc_html (preg_replace ("/^" . preg_quote ($_SERVER["DOCUMENT_ROOT"], "/") . "/", "", dirname (ABSPATH) . "/wp-config.php")) . '</code>. Quick Cache needs write-access to this file. Permissions need to be <code>755</code> or higher.', true);
					}
				if (is_dir (WP_CONTENT_DIR) && !is_writable (WP_CONTENT_DIR))
					{
						ws_plugin__qcache_display_admin_notice ('<strong>Permissions:</strong> Please check permissions on <code>' . esc_html (preg_replace ("/^" . preg_quote ($_SERVER["DOCUMENT_ROOT"], "/") . "/", "", WP_CONTENT_DIR)) . '</code>. Quick Cache needs write-access to this directory. Permissions need to be <code>755</code> or higher.', true);
					}
				if (is_dir (WP_CONTENT_DIR . "/cache") && !is_writable (WP_CONTENT_DIR . "/cache"))
					{
						ws_plugin__qcache_display_admin_notice ('<strong>Permissions:</strong> Please check permissions on <code>' . esc_html (preg_replace ("/^" . preg_quote ($_SERVER["DOCUMENT_ROOT"], "/") . "/", "", WP_CONTENT_DIR . "/cache")) . '</code>. Quick Cache needs write-access to this directory. Permissions need to be <code>755</code> or higher.', true);
					}
				/**/

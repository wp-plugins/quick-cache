...
*/
if (!function_exists ("ws_plugin__qcache_parse_readme"))
	{
		function ws_plugin__qcache_parse_readme ($specific_path = FALSE, $specific_section = FALSE, $_blank_targets = TRUE, $process_wp_syntax = FALSE)
			{
				...
				if (file_exists ($path)) /* Give hooks a chance. */
					{
						...
						if ($specific_section) /* If we are ONLY parsing a specific section. This is a very useful way of pulling details out. */
						...
						else /* Otherwise, we're going for the entire readme file. Here we have lots of work to do. */
							{
								...
								$rm = stripslashes (preg_replace ($r3, "'$1$2$3$4$5'._ws_plugin__qcache_parse_readme_specs('$6').'$7'", $rm, 1));
...
*/
if (!function_exists ("_ws_plugin__qcache_parse_readme_specs"))
	{
		function _ws_plugin__qcache_parse_readme_specs ($str = FALSE)
			{
...
*/
if (!function_exists ("ws_plugin__qcache_parse_readme_value"))
	{
		function ws_plugin__qcache_parse_readme_value ($key = FALSE, $specific_path = FALSE)
			{

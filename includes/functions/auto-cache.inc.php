...
*/
if (!function_exists ("ws_plugin__qcache_add_auto_cache_engine"))
	{
...
*/
if (!function_exists ("ws_plugin__qcache_delete_auto_cache_engine"))
	{
		function ws_plugin__qcache_delete_auto_cache_engine ()
			{
				...
				if (function_exists ("wp_cron")) /* If WP-Cron is available. */
					{
						wp_clear_scheduled_hook ("qcache_cron_auto_cache_engine"); /* For backward compatibility. */
						wp_clear_scheduled_hook ("ws_plugin__qcache_auto_cache_engine__schedule"); /* Since 2.1.8. */
						/**/
...
*/
if (!function_exists ("ws_plugin__qcache_auto_cache_engine"))
	{
												...
												if (preg_match ("/^http(s?)\:\/\/(.*?)" . preg_quote ($_SERVER["HTTP_HOST"], "/") . "\//i", $url))
													{
														if ($host_uri = preg_replace ("/^http(s?)\:\/\//i", "", $url))
															{
																$md5_cache = WP_CONTENT_DIR . "/cache/" . md5 ($salt_host_uri = QUICK_CACHE_VERSION_SALT . $host_uri);
																/**/

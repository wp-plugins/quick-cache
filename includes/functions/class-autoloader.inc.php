...
/*
The __autoload function for Quick Cache classes.
This highly optimizes Quick Cache. Giving it a much smaller footprint.
See: http://www.php.net/manual/en/function.spl-autoload-register.php
*/
if (!function_exists ("ws_plugin__qcache_classes")) /* Already exists? */
	{
		function ws_plugin__qcache_classes ($class = FALSE) /* Build dynamic __autoload function. */
			{
				...
				/**/
				if (strpos ($class, "c_ws_plugin__qcache_") === 0 && strpos ($class, "c_ws_plugin__qcache_pro_") === false)
					{
						$c = (!isset ($c)) ? dirname (dirname (__FILE__)) . "/classes" : $c; /* Configures location of classes. */
						$c_class_dirs = (!isset ($c_class_dirs)) ? array_merge (array ($c), _ws_plugin__qcache_classes_scan_dirs_r ($c)) : $c_class_dirs;
						/**/
						$class = str_replace ("_", "-", str_replace ("c_ws_plugin__qcache_", "", $class));
						/**/
		...
		function _ws_plugin__qcache_classes_scan_dirs_r ($starting_dir = FALSE)
			{
					...
					if (is_dir ($starting_dir)) /* Does this directory exist? */
						foreach (scandir ($starting_dir) as $dir) /* Scan this directory. */
							if ($dir !== "." && $dir !== ".." && is_dir ($dir = $starting_dir . "/" . $dir))
								$dirs = array_merge ($dirs, array ($dir), _ws_plugin__qcache_classes_scan_dirs_r ($dir));
				/**/
				return $dirs; /* Return array of all directories. */
			}
		/**/
		spl_autoload_register ("ws_plugin__qcache_classes"); /* Register __autoload. */
	}
?>
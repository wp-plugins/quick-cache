...
*/
if (!function_exists ("ws_plugin__qcache_add_advanced"))
	{
		function ws_plugin__qcache_add_advanced ()
			{
				do_action ("ws_plugin__qcache_before_add_advanced", get_defined_vars ());
				/**/
				if (!ws_plugin__qcache_delete_advanced ()) /* Do not proceed if unable to delete. */
					{
				...
				else if (is_writable (WP_CONTENT_DIR) && (!file_exists (WP_CONTENT_DIR . "/advanced-cache.php") || is_writable (WP_CONTENT_DIR . "/advanced-cache.php")) && ($handler = file_get_contents (dirname (dirname (__FILE__)) . "/templates/handler.txt")))
					{
						...
						foreach (preg_split ("/[\r\n\t]+/", $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["dont_cache_these_uris"]) as $uri)
							if ($uri = trim ($uri))
								$uris .= "|" . preg_quote ($uri, "/");
						if ($uris = trim ($uris, " \r\n\t\0\x0B|"))
							$uris = "/" . ws_plugin__qcache_esc_dq ($uris) . "/";
						...
						foreach (preg_split ("/[\r\n\t]+/", $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["dont_cache_these_refs"]) as $ref)
							if ($ref = trim ($ref))
								$refs .= "|" . preg_quote ($ref, "/");
						if ($refs = trim ($refs, " \r\n\t\0\x0B|"))
							$refs = "/" . ws_plugin__qcache_esc_dq ($refs) . "/i";
						...
						if ($agents = trim ($agents, " \r\n\t\0\x0B|"))
							$agents = "/" . ws_plugin__qcache_esc_dq ($agents) . "/i";
						...
						if (strlen ($GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["version_salt"]))
							if (file_put_contents (WP_CONTENT_DIR . "/qcache-salt-ok.php", '<?php error_reporting(0); $v = ' . $GLOBALS["WS_PLUGIN__"]["qcache"]["o"]["version_salt"] . '; echo "ok"; ?>')):
								$salt_ok = (($syntax_check = ws_plugin__qcache_remote (WP_CONTENT_URL . "/qcache-salt-ok.php")) === "ok") ? true : false;
...
*/
if (!function_exists ("ws_plugin__qcache_delete_advanced"))
	{
